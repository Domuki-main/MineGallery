...
class Database {
  ...
  clearImageFromAlbum2(String imageId) async {
    List connection = [];
    await albums_images
  ...
  Future<bool> isAlbumDuplicated(String title) async {
    bool result = false;
    await albumCollection.where('title', isEqualTo: title).get().then((val) => {
          val.docs.forEach((doc) {
            print("doc.get('title')");
            print(doc.get('title'));
            result = true;
  ...
  Future<bool> isConnDuplicated(String imageId) async {
    bool result = false;
    await albums_images
        .where('imageId', isEqualTo: imageId)
        .get()
  ...
  // isImageDuplicated(String id) async {
  //   bool result = false;
  //   await imageCollection.where('albumId', isEqualTo: id).get().then((val) => {
  //         val.docs.forEach((doc) {
  //           result = true;
  //         }),
  //       });
